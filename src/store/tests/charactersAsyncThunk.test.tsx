import { fetchIssues } from "../slices";
jest.setTimeout(20000);

global.fetch = jest.fn();

describe("Issues Thunk", () => {
  it("should fetch with rejected response", async () => {
    const dispatch = jest.fn();
    const thunk = fetchIssues({ url: "" });
    ...
    expect(start[0].type).toBe("issues/fetchIssues/pending");
    expect(end[0].type).toBe("issues/fetchIssues/rejected");
  });
  ...
  it("should fetch with resolved response", async () => {
    const mockIssues = [
      {
        ...
        user: {
          ...
          starred_url:
            "https://api.github.com/users/leadq/starred{/owner}{/repo}",
          ...
          received_events_url:
            "https://api.github.com/users/leadq/received_events",
          type: "User",
          site_admin: false,
        },
        labels: [
    ...
    expect(start[0].type).toBe("issues/fetchIssues/pending");
    expect(end[0].type).toBe("issues/fetchIssues/fulfilled");
    expect(end[0].payload).toBe(mockIssues);
  });
});
